Actions.challengeTiger = {
	name = 'challengeTiger',
}

function Actions.challengeTiger:doAction(roleid, args)
	local jrole = _GetOnline(roleid)
	if jrole then
		local goldForCycles = args.goldForCycles * 100
		...
		--添加到发起挑战列表了
		_ChallengeAddCyclesMessage( roleid, goldForCycles, 0)
		--发送消息
