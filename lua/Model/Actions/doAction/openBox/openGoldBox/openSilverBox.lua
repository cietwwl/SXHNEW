...
function Actions.openSilverBox:doAction(roleid, args)
	--获得角色对象
	...
	if jrole then
		...
		--要扣除的金钱
		local deductMoney = self.silver
		--宝箱开启状态
		local boxIsOpened = _TaskStateExtraGetI(task_state, 6)
		------------------------------------------------------------------------------------
		...
		--设置宝箱开启状态			
		_TaskStateExtraSetI(task_state, 6, 1)
		--设置冷却时间
		...
		log.item("" .. _RoleGetUserId(jrole) .."#$" .. _RoleGetId(jrole) .. "#$23#$" .. ItemSet[itemid].name .. "#$" .. num .. "#$"..os.date("%Y-%m-%d %H:%M:%S").."#$"..TianLongServer.srvId.."#$".. 0 .."#$".._Long2String(tid).."#$".._Long2String(uid))
		end
		--同步人物基本属性
		fillAttributes(jrole)
		
		--//TODO缺少金钱减少日志
		log.item("" .. _RoleGetUserId(jrole)  .."#$" .. _RoleGetId(jrole) .. "#$26#$本次开宝箱获得物品#$获得物品所有个数#$"..os.date("%Y-%m-%d %H:%M:%S").."#$"..TianLongServer.srvId.."#$" .. deductMoney .. "")
