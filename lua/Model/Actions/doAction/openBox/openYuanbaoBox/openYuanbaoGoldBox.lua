--开启金币宝箱
Actions.openYuanbaoGoldBox = {
	name = 'openYuanbaoGoldBox',
	--金宝箱扣除元宝数目,当前为12元宝
	gold = 100,
	--金宝箱物品储存表
	goldBoxId = 5079,
...
function Actions.openYuanbaoGoldBox:doAction(roleid, args)
	--获得角色对象
	...
	if jrole then
		...
		--要扣除的元宝
		local deductYuanbao = self.gold
		--宝箱开启状态(0为未开启，1为已开启)
		local boxIsOpened = _TaskStateExtraGetI(task_state, 9)
		...
		--扣除开宝箱所需元宝
		local gameSubtractInfo = luajava.new(GameSubtract, _RoleGetUserId(jrole), _RoleGetJoyId(jrole), deductYuanbao, 0, 0, deductYuanbao, 0, yuanBaoOpConst.OPEN_YUANBAO_GOLD_BOX) 
        --//TODO缺少元宝减少日志
	    log.item("" .. _RoleGetUserId(jrole)  .."#$" .. _RoleGetId(jrole) .. "#$33#$本次开宝箱获得物品#$获得物品所有个数#$"..os.date("%Y-%m-%d %H:%M:%S").."#$"..TianLongServer.srvId.."#$" .. deductYuanbao .. "")
...
function Actions.openYuanbaoGoldBox:randomItems(jrole)
	--获得角色已接受任务列表（必须保证该任务已接受）
	local role_task = _GetRoleTask(jrole)
	--获得角色该宝箱任务
	local task_state = _GetTaskState(role_task, 21030)
	...
	for _, v in ipairs(itemsTbl) do
		...
		log.item("" .. _RoleGetUserId(jrole) .."#$" .. _RoleGetId(jrole) .. "#$30#$" .. ItemSet[itemid].name .. "#$" .. num .. "#$"..os.date("%Y-%m-%d %H:%M:%S").."#$"..TianLongServer.srvId.."#$".. 0 .."#$".._Long2String(tid).."#$".._Long2String(uid))
	end	
	--同步人物基本属性（能否同步元宝）
...
function Actions.openYuanbaoGoldBox:YuanbaoSyntony(jrole, isSuccess)
