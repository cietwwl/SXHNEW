--开启金币宝箱
Actions.openYuanbaoCopperBox = {
	name = 'openYuanbaoCopperBox',
	--铜宝箱扣除元宝数目，当前为5渊博啊
	copper = 10,
	--铜宝箱物品储存表
	copperBoxId = 5083,
...
function Actions.openYuanbaoCopperBox:doAction(roleid, args)
	--获得角色对象
	...
	if jrole then
		...
		--要扣除的元宝
		local deductYuanbao = self.copper
		--宝箱开启状态(0为未开启，1为已开启)
		local boxIsOpened = _TaskStateExtraGetI(task_state, 11)
		...
		--角色剩余元宝(扣除开宝箱所需元宝)
		local gameSubtractInfo = luajava.new(GameSubtract, _RoleGetUserId(jrole), _RoleGetJoyId(jrole), deductYuanbao, 0, 0, deductYuanbao, 0, yuanBaoOpConst.OPEN_YUANBAO_COPPER_BOX) 
        --//TODO缺少元宝减少日志
	    log.item("" .. _RoleGetUserId(jrole)  .."#$" .. _RoleGetId(jrole) .. "#$31#$本次开宝箱获得物品#$获得物品所有个数#$"..os.date("%Y-%m-%d %H:%M:%S").."#$"..TianLongServer.srvId.."#$" .. deductYuanbao .. "")
		PayManager:postSubTask(_RoleGetYuanBaoOp(jrole), gameSubtractInfo)

	end
end

function Actions.openYuanbaoCopperBox:randomItems(jrole)
	--获得角色已接受任务列表（必须保证该任务已接受）
	local role_task = _GetRoleTask(jrole)
	--获得角色该宝箱任务
	local task_state = _GetTaskState(role_task, 21030)
	--随机物品
	local boxToBeOpened = ItemSet[self.copperBoxId]
	...
	--设置宝箱开启状态			
	_TaskStateExtraSetI(task_state, 11, 1)
	--元宝宝箱当前储存的冷却时间
	local coolTimeTemp =_TaskStateExtraGetI(task_state, 8)
	--取系统当前时间
	local systemTime = _GetMinute()
	...
	for _, v in ipairs(itemsTbl) do
		...
		log.item("" .. _RoleGetUserId(jrole) .."#$" .. _RoleGetId(jrole) .. "#$28#$" .. ItemSet[itemid].name .. "#$" .. num .. "#$"..os.date("%Y-%m-%d %H:%M:%S").."#$"..TianLongServer.srvId.."#$".. 0 .."#$".._Long2String(tid).."#$".._Long2String(uid))
	end	
	--同步人物基本属性（能否同步元宝）
...
function Actions.openYuanbaoCopperBox:YuanbaoSyntony(jrole, isSuccess)
