...
Actions.betBigOrSmall = {
	name = 'betBigOrSmall',
}

function Actions.betBigOrSmall:doAction(roleid, args)
	
	local jrole = _GetOnline(roleid)
	--押大小扣除的手续费百分比
	--local commission = 0.1
	--押大的金额
	...
	if goldForBig < 0 or goldForSmall < 0 then
		self:error(jrole, "押注金额不能为负", "bigOrSmall")
		return 
	end
	...
	if jrole then
		...
		if goldForBig < 100 and goldForSmall < 100 then
			...
			self:error(jrole, "押注最小金额为1银币", "bigOrSmall")
			return 
		end
		
		if roleGold < goldForBig + goldForSmall then
			--返回金币不足
			self:error(jrole, "太可惜了，您的现金不足。", "bigOrSmall")
			return
		end
		...
		if goldForSmall >= 1 then 
			--_BigOrSmallAddBetRole(roleid, goldForSmall * (1 - commission), 0)
			_BigOrSmallAddBetRole(roleid, goldForSmall, 0)
		end
		--调用押小的处理机制 
		if goldForBig >= 1 then 
			--_BigOrSmallAddBetRole(roleid, goldForBig * (1 - commission), 1)
			_BigOrSmallAddBetRole(roleid, goldForBig, 1)
		end
		self:commonMessage(jrole, "恭喜您！押注成功", "bigOrSmall")
		---------------------------------------------------------------------------------------
		----同步客户端数据
		--清空缓冲区
		prepareBody()
