...
function _ShedAdd:canUse(jrole)
	return true
end

function _ShedAdd:useItem(jrole, item)
	---------------------------------------------------------------------------
	--条件验证
	local role_store = _RoleGetStore(jrole)
	if not role_store:canAddShed() then
		replyMessage(jrole, 2, MsgID.MsgID_Item_Do_Resp, "仓库已经达到上限，不能再开通！")
		return
	end
	--条件验证
	---------------------------------------------------------------------------
	...
	---------------------------------------------------------------------------
	--返回提示
	local depot_count = role_store:getDepotCount()
	sendMsg(jrole, MsgID.MsgID_Item_Do_Resp, "已使用" .. self.name .. "/目前已开通仓库数：" .. depot_count)
	--返回提示
	---------------------------------------------------------------------------
	--同步物品与背包属性
	prepareBody()
...
function _ShedAdd:getAuctionCategory()
